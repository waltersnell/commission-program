...
- Administrator: manage settings through four responsive submenus. Users contains Create User Access and Users; Commission Settings contains Commissionable Staff, Add Commissionable Staff, and Commission Settings; Other Settings contains CRM Steps, Locations, and Membership Types; Client Search contains Client Lookup and Audit History. The selected submenu remains active through searches, edits, saves, and validation redirects, and its panels stay visible.
...
- Client Lookup: lists First Visit and Sale Date independently. Administrators can correct sale dates, locations, and closer assignments in the client editor. Sold records synchronize their final closer assignments and credit rows, first-visit eligibility is recalculated, and sale changes are written to Audit History.
...
- Opportunities: administrators see filterable open Hot/Warm opportunities; non-admin users see every matching opportunity in `My Opportunities` followed by every other open opportunity in `All Other Opportunities`, with each row linking to editable opportunity detail. Next Action is currently `Personal SMS` with Hot due first-visit +1 day and Warm due first-visit +2 days.
