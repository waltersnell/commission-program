...
- Legacy opportunity progress remains compatible with that workflow. Historical Personal SMS completion/Phone Outreach and Need 2nd/3rd Followup values are interpreted as completed earlier steps, while new completions continue to store stable `crmStepId` values.
- User-facing dates and exported payroll dates are `MM/DD/YYYY`. Admin Client Search supports audited membership-type corrections for existing sales, and Opportunities shares the tokenized full-name/normalized-phone lookup used by Client Search.
